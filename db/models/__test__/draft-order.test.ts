...
describe('Test DraftOrderModel methods and static functions', () => {
    describe('getAvailablePositionsByLigueGroup', () => {
    ...
    describe('resetDraftOrder', () => {
        it('Should set isSelected to false for all spots in all documents', async () => {
            const dolceGroup = await new LigueGroupsModel({
                groupName: 'dolce'
            }).save();
            ...
            await DraftOrderModel.resetDraftOrder();

            const documents = await DraftOrderModel.find().exec();

            expect(documents).toHaveLength(2);
            documents.forEach(document => {
                document.spot.forEach(spot => {
                    expect(spot.isSelected).toBe(false);
                });
            });
        });
    });
